...
export default function Footer() {
  ...
  // 🚀 FUNCIÓN DE SCROLL OPTIMIZADA PARA COMPONENTES <LINK>
  const volverArribaDelTodo = () => {
    // Le damos 50ms al router para cambiar de página antes de lanzar la animación del scroll
    setTimeout(() => {
      window.scrollTo({
        top: 0,
        behavior: 'smooth'
      ...
      <div className="max-w-6xl mx-auto px-6 py-16 grid gap-10 sm:grid-cols-2 lg:grid-cols-4 text-left relative z-10">
        ...
        <div className="space-y-4">
          <div className="flex-shrink-0 h-28 sm:h-34 w-42 sm:w-48 overflow-hidden group">
            {/* 💡 Agregamos la función al Link del Logo para que también suba de forma fluida */}
            <Link to="/" onClick={volverArribaDelTodo} className="block w-full h-full">
